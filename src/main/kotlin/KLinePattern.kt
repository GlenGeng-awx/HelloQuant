data class KLinePattern(val kLine: KLine) {
    fun doubleBottom(idx: Int, range1: Int, range2: Int): Boolean {
        if (!KLineOpByIdx(kLine).lowestIn(idx, range1, range2)) {
            return false
        }
        ...
        return KLineOpByIdx(kLine).fallTo(lowPlus3Percent, idx, range2)
    }

    fun doubleTop(idx: Int, range1: Int, range2: Int): Boolean {
        if (!KLineOpByIdx(kLine).highestIn(idx, range1, range2)) {
            return false
        }
        ...
        return KLineOpByIdx(kLine).riseTo(highMinus3Percent, idx, range2)
    }

    // idx is local min,
    // achieve a higher price every day for 3 consecutive days starting from idx + 1
    fun reversalUpwards(idx: Int): Boolean {
        if (!KLineOpByIdx(kLine).localMin(idx)) {
            return false
        }

        return KLineOpByIdx(kLine).consecutiveHigherPrice(idx, 3)
    }
